...
public class MvcControllerHandler extends ResponseEntityExceptionHandler {
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex,
		...
		ex.getBindingResult().getFieldErrors().stream().forEach(error -> errorList.add("Parameter: "+error.getField()+ " " + error.getDefaultMessage()));
		return new ResponseEntity<Object>(errorList, HttpStatus.BAD_REQUEST);
	}

	@ExceptionHandler({ CpfExistenteException.class, PessoaNaoEncontradaException.class })
	public ResponseEntity<String> errosNegocioHandler(RuntimeException e) {
		HttpStatus status;
		if (e instanceof CpfExistenteException) {
